...
const SearchUsersModal = ({
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      title="Search users"
      description="Find existing users through the live backend search endpoint and open a conversation shell."
      size="lg"
    >
          ...
          <div className="flex min-h-[280px] items-center justify-center">
            {query.trim().length >= 2 ? (
              <EmptyState
                ...
                description="Try another username or shorten the query to broaden the live results."
              />
            ) : (
              <EmptyState
                icon={UserRoundPlus}
                title="Search the user directory"
                description="Type at least 2 characters to query the existing backend user search route."
              />
            )}
          </div>
        )}
